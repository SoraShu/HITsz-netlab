...
void arp_in(buf_t *buf, uint8_t *src_mac) {
    ...
    // if there is a ip packet waiting for this ip
    // send it out
    buf_t *ip_buf = (buf_t *)map_get(&arp_buf, arp_pkt->sender_ip);
    if (ip_buf != NULL) {
        ethernet_out(ip_buf, arp_pkt->sender_mac, NET_PROTOCOL_IP);
        // TODO: will change when fix packet loss
        map_delete(&arp_buf, arp_pkt->sender_ip);
    }

    // if this is a arp request, send a arp response
